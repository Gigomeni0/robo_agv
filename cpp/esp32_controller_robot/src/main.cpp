...
void verificarSensores() {
  if (millis() - ultimaLeituraSensores >= INTERVALO_LEITURA_SENSORES) {
    ...
    if ((distanciaSecundaria1 <= DISTANCIA_SEGURANCA && distanciaSecundaria1 > 0) || 
        (distanciaSecundaria2 <= DISTANCIA_SEGURANCA && distanciaSecundaria2 > 0)) {
      if (emMovimento) {
        pararMotores();
        mqttClient.publish("robo/status", "ALERTA: Obstaculo detectado");
      }
    }
    
    String leituras = "Sensores: " + String(distanciaPrioritaria) + "cm, " + 
                     String(distanciaSecundaria1) + "cm, " + 
                     String(distanciaSecundaria2) + "cm";
    mqttClient.publish("robo/sensores", leituras.c_str());
  }
}
...
void processarFila() {
  if (emMovimento || filaMensagens.empty()) return;
